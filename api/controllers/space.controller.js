...
async function GET(req,res){
  ...
  var filters = {};
  var fields = "";
  var limit = config.space_limit;

  if(req.query.limit != undefined || req.query.limit != null){
    limit = req.query.limit;
    if(limit < 0 || limit > config.space_limit){
      limit = config.space_limit
    }
  }
  ...
  try{
    var body = await spaceService.getAll(Number(limit),filters,fields);
...
async function GET_ID(req,res){
  ...
  try{
    var body = await spaceService.getById(id);
    return res.json(body);
  }catch (err){
    log.error(err);
    return res.status(500).send({ error: 'Fire in the hole' });
...
async function POST(req,res){
  ...
  try{
    var body = await spaceService.create(req);
    return res.json(body);
  }catch (err){
    log.error(err);
    return res.status(500).send({ error: 'Fire in the hole' });
...
async function DELETE_ID(req,res){
  ...
  try{
    var body = await spaceService.deleteById(id);
    return res.json(body);
  }catch (err){
    log.error(err);
    return res.status(500).send({ error: 'Fire in the hole' });
...
async function PUT_ID(req,res){
  ...
  try{
    var body = await spaceService.updateById(id, req);
    return res.json(body);
  }catch (err){
    log.error(err);
    return res.status(500).send({ error: 'Fire in the hole' });
